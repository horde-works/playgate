...
const VICTIM = argOf("--victim", "combat-hexacopter-range:vehicle:engine:0:blade:0:piece");
  ...
  try {
    ...
    const speedOf = async () => {
      const raw = await cdp.eval(
        `JSON.stringify((window.__mamVehicles?.() ?? []).find((v) => v.id === ${JSON.stringify(VEHICLE)})?.body ?? null)`,
      );
      const b = JSON.parse(raw ?? "null");
      if (!b) return null;
      const v = b.velocity ?? b.linvel ?? null;
      return v ? Math.hypot(v[0], v[1], v[2]) : null;
    };

    const speedsBefore = [];
    for (let i = 0; i < 4; i += 1) {
      speedsBefore.push(await speedOf());
      await sleep(400);
    }
    console.log("скорость ДО облома:", speedsBefore.map((v) => (v ?? NaN).toFixed(1)).join(" "));

    console.log("ломаю лопасть:", VICTIM);
    console.log("ответ хука:", await cdp.eval(
    ...
    for (let i = 0; i < 16; i += 1) {
      await sleep(500);
      speedsAfter.push(await speedOf());
    }
    console.log("скорость ПОСЛЕ:", speedsAfter.map((v) => (v ?? NaN).toFixed(1)).join(" "));

    const tail = speedsAfter.slice(-6).filter((v) => v !== null);
    const stalled = tail.length > 0 && tail.every((v) => v < 0.6);
    console.log(
      stalled
        ? "\nВЕРДИКТ: машина ВСТАЛА после облома — зацепилась за собственный кусок."
        : "\nВЕРДИКТ: машина продолжает идти, зацепа не видно.",
    );
  } finally {
    if (cdp) {
      try {
