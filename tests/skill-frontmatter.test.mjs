...
//
// Случай, ради которого написано: заглушка объектного скилла с абсолютным
// путём макового чекаута. На Windows чекаут лежит по другому пути, и там эта
// заглушка не разрешилась бы ни во что, продолжая перекрывать по имени. Дрейф
// к тому моменту уже случился — в agents/openai.yaml разошлись
// short_description и default_prompt, тогда как frontmatter обеих копий
// совпадал посимвольно, то есть глазами не ловился.
//
// Сами пути живут в parallel-work §1 и не цитируются здесь: за этим следит
// tests/portable-paths.test.mjs.
//
