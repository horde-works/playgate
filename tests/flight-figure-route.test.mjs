...
test("фигуры доходят до машины ИМЕННО ТЕМ маршрутом, который она полетит", () => {
  ...
  // ПРИБЫТИЕ — ЭТО ПОСАДКА, А НЕ ПРОГРАММА ПОКАЗА, и прежнее решение здесь
  // отменено (вердикт Igor, 11.08.2026). Раньше заходом служил тот же
  // показательный круг: машину переносили в его начало, и она уходила крутить
  // номера вместо того, чтобы сесть, — снаружи это читалось как «появился на
  // площадке и взлетел, не приземлившись», а иногда уводило её за остров.
  //
  // Теперь заход это прямая с горизонта к берту со снижением, и фигур на ней
  // нет и быть не должно: возвращающаяся машина садится.
  const arrival = rax8.flight.arrivalPlan(BERTH);
  assert.equal(arrival.figures, undefined, "на заходе на посадку крутят петли");
  // И приходит она С РАЗНЫХ СТОРОН: створ — довод, а не константа, иначе
  // подменные суда идут конвейером в один и тот же коридор.
  const east = rax8.flight.arrivalPlan(BERTH, { bearing: Math.PI / 2 });
  const west = rax8.flight.arrivalPlan(BERTH, { bearing: -Math.PI / 2 });
  assert.notEqual(
    east.point(0)[0].toFixed(1),
    west.point(0)[0].toFixed(1),
    "створ захода не зависит от пеленга",
  );
  // А отзыв с пульта строит тот же заход от ТЕКУЩЕГО места машины.
  const recalled = rax8.flight.arrivalPlan(BERTH, { from: [40, 30, -20] });
  assert.deepEqual(
    recalled.point(0).map((value) => Number(value.toFixed(1))),
    [40, 30, -20],
  );
});
