...
test("villagers cannot walk through the world", () => {
  ...
  for (let tick = 0; tick < 9000; tick += 1) {
    ...
    for (const villager of population.villagers) {
      ...
      walkFrames += 1;
      for (const box of field.query(villager.x, villager.z, 1, population.broken)) {
  ...
  // Идеального нуля не бывает: кадр соприкосновения на углу возможен, но
  // «ходить сквозь стены» — это проценты, а не сотые доли.
  const share = insideFrames / Math.max(walkFrames, 1);
  assert.equal(share < 0.002, true, `inside solids ${(100 * share).toFixed(2)}% (${worst})`);
});
