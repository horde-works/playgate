...
test("НЕИЗВЕСТНЫЙ ВИД ТРАССЫ ОТКАЗЫВАЕТ ГРОМКО, а не отдаёт пустоту", () => {
  // Живая проба попросила у машины полигона вид `lap`, которого у неё нет.
  // Селектор тихо вернул `undefined`, отправка ответила «взлетела», и падало
  // уже в шаге физики — КАЖДЫЙ КАДР, до перезагрузки страницы.
  //
  // Тип от этого не спасает: вид приходит и снаружи типов — из dev-хука
  // отправки, из сохранённого состояния, из чужой карты. Ошибка обязана
  // звучать там, где имя ещё есть и его можно понять.
  assert.throws(
    () => rangeHexacopterRoute("lap"),
    (error) => {
      assert.match(error.message, /lap/);
      // В сообщении обязателен СПИСОК доступного: без него следующий человек
      // будет гадать так же.
      assert.match(error.message, /circuit/);
      return true;
    },
  );
  // Законные виды по-прежнему отдают трассу.
  for (const kind of ["circuit", "tour", "evasive"]) {
    assert.ok(rangeHexacopterRoute(kind), `вид ${kind} перестал существовать`);
  }
});
