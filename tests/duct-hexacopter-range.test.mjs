...
test("VX-8: машина и её пад собираются в кластеры полигона", () => {
  ...
  // 779, а не 715: ротор перестал быть одним куском — у каждой лопасти свой
  // (11.08.2026). Плата за постепенную потерю тяги вместо потери стороны.
  assert.equal(vehicle.pieces.length, 779);
});
...
test("VX-8: маски кадра ловят реальные куски, а не пустоту", () => {
  ...
  // По лопасти, а не по вентилятору: шесть колец по десять и два тоннеля по
  // шесть. Восьмёрка здесь означала бы, что ротор снова один кусок.
  assert.equal(blades.length, 72, "по лопасти на кусок, а не по вентилятору");
