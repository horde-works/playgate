...
test("показательный маршрут уходит за остров, вьётся в обе стороны и возвращает машину на место взлёта", () => {
  ...
  // ЗНАК КРИВИЗНЫ ОБЯЗАН МЕНЯТЬСЯ, И НЕ РАЗ.
  //
  // Здесь проверялась восьмёрка на участке перед заходом: у неё знак меняется
  // в середине по построению, и это была единственная гарантия, что маршрут не
  // выродился в круг. Восьмёрки в программе больше нет — её заменили одиннадцать
  // номеров, — но САМО ТРЕБОВАНИЕ осталось тем же и стало шире: круг, при всей
  // его невинности, гнётся в одну сторону, и таким его и видно. Считается теперь
  // по всей программе, а не по одному участку.
  const turn = (progress) => {
  ...
  for (let index = 1; index < 240; index += 1) {
    const at = 0.05 + (index / 240) * 0.9;
    let delta = turn(at) - turn(at - 0.003);
    while (delta > Math.PI) delta -= Math.PI * 2;
    while (delta < -Math.PI) delta += Math.PI * 2;
    const sign = delta > 0.002 ? 1 : delta < -0.002 ? -1 : 0;
    if (sign > 0) clockwise += 1;
    if (sign < 0) counter += 1;
    if (sign !== 0 && previousSign !== 0 && sign !== previousSign) reversals += 1;
    if (sign !== 0) previousSign = sign;
  }
  // РАВНОВЕСИЯ ЗНАКОВ ТРЕБОВАТЬ НЕЛЬЗЯ, И ЭТО ГЕОМЕТРИЯ, А НЕ СНИСХОЖДЕНИЕ.
  //
  // Прежний порог «двенадцать витков в каждую сторону» был написан под маршрут,
  // содержавший восьмёрку. У всякой ЗАМКНУТОЙ линии суммарный поворот равен
  // ровно ±360°, поэтому один знак в ней преобладает всегда — если только она
  // не пересекает саму себя, как восьмёрка. Требовать равновесия значит
  // требовать восьмёрку, а её в программе показа больше нет.
  //
  // Что проверять осмысленно — СМЕНЫ ЗНАКА: именно они отличают программу от
  // орбиты, и именно они требуют от машины переложить рыскание через ноль на
  // ходу. Замер: 155 витков по часовой против 10 и четыре смены знака.
  assert.ok(counter > 5, `ни одного витка против часовой: ${counter}`);
  assert.ok(
    reversals >= 3,
    `знак кривизны меняется всего ${reversals} раз — это орбита, а не программа`,
  );
  // И удаление от площадки гуляет: программа обходит остров, а не наматывает
  // орбиту постоянного радиуса.
