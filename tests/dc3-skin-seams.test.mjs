...
test("the fuselage runs the lattice along its own longest axis", () => {
  const group = createDc3AirplaneGroup(placement);
  // Обшивка фюзеляжа — тоже панели. Закон в шейдере выбирает ось по самому
  // длинному размеру КУСКА: у продольного клина это нос-хвост, поэтому
  // кольца встают поперёк борта, а полосы идут вдоль. Проверяется на всех.
  // ГРАНИЦА ЗАКОНА, А НЕ ИСКЛЮЧЕНИЕ ПО УДОБСТВУ.
  //
  // Ось решётки выбирается по самому длинному размеру КУСКА. Пока клин шёл
  // через весь отсек, продольная ось побеждала всегда. Врезка иллюминаторов
  // нарезала клин на короткие сегменты и обкладку проёма — у них длинная ось
  // поперечная ПО ПОСТРОЕНИЮ, и решётка там ложится кольцами вдоль борта.
  // Это видно вблизи на обкладке окна и есть известное ограничение закона:
  // чинится оно не тестом, а тем, что ось решётки должна приходить от ТЕЛА,
  // а не от куска. Пока закон покрывает целые клинья.
  const gores = group.objects.filter((object) =>
    /^fuselage:bay\d+:gore\d+$/.test(object.id));
  assert.ok(gores.length > 30, `целых клиньев фюзеляжа всего ${gores.length}`);
  for (const gore of gores) {
    const [sx, sy, sz] = gore.size;
    assert.ok(
      sz > sx && sz > sy,
      `${gore.id}: длинная ось не продольная (${sx.toFixed(2)}/${sy.toFixed(2)}/${sz.toFixed(2)})`,
    );
  }

  // ОБШИВКА КРЫЛА ТЕПЕРЬ ПАНЕЛИ, И ЗАКОН ОБЯЗАН ВЫЖИТЬ НА КАЖДОЙ.
  //
  // Ось решётки выбирается по самому длинному размеру КУСКА. Пока крыло было
  // одной шкурой, размах побеждал заведомо. У панели 1.6 м по размаху против
  // полосы по хорде — и если бы полоса оказалась шире, на этой панели ряды
  // заклёпок легли бы поперёк соседних. Проверяется на всех, а не на одной.
  const panels = group.objects.filter((object) => /^wing-(left|right):/.test(object.id));
  assert.ok(panels.length > 40, `панелей крыла всего ${panels.length}`);
  for (const panel of panels) {
    assert.ok(
      panel.size[0] > panel.size[2],
      `${panel.id}: хорда ${panel.size[2].toFixed(2)} шире размаха ${panel.size[0].toFixed(2)} — решётка ляжет поперёк`,
    );
  }
});
