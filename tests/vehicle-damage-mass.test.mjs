...
test("огрызок лёгкой панели наследует её авторский вес", () => {
  // Лист обшивки авторится вчетверо легче своего габарита. Ядро carve
  // считает объём огрызка по геометрии, поэтому его надо привести к той же
  // авторской плотности — иначе дырка в борту делает корабль тяжелее.
  const panel = { size: [2, 1, 0.2], volume: 0.1 }; // габарит 0.4, авторский 0.1
  const boundingVolume = panel.size[0] * panel.size[1] * panel.size[2];
  const scale = Math.min(1, panel.volume / boundingVolume);
  assert.equal(scale, 0.25);

  const kernelStumpVolume = 0.24; // ядро отдало геометрический объём огрызка
  const scaled = kernelStumpVolume * scale;
  assert.ok(
    scaled < panel.volume,
    `огрызок обязан быть легче целой панели: ${scaled} против ${panel.volume}`,
  );
});
