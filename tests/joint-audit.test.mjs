 ...
 *     она висит на 10 мм над своей плитой (`floorBase + 0.01`), и щель под
 *     ней шла по периметру каждой комнаты каждого этажа.
 *
 * Отсюда рост town 459 → 3742 и basalt 5 → 122: это не регрессия, это конец
 * слепоты. Числа снова «не расти».
 */
const BUDGET = {
  town: { seeThrough: 3742, fights: 117 },
