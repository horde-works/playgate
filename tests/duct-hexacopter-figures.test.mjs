...
test("КУЛЬБИТ уходит ТЕМ ЖЕ КУРСОМ — в этом вся разница с иммельманом", () => {
  ...
  // ПОРОГ СДВИНУТ ВМЕСТЕ С МАШИНОЙ, а не подогнан под прогон.
  //
  // Лопасти VX-8 стали отдельными кусками (11.08.2026), и масса ротора
  // переехала из фантомной коробки в ступице на НАСТОЯЩИЙ радиус 0.43 м.
  // Инерция крена от этого честно выросла, и та же фигура выходит с 11.7°
  // остаточного крена вместо 11.5°. Прежний порог 0.98 описывал машину, у
  // которой десять лопастей весили как точка в центре втулки.
  //
  // Замер после раскола: 0.979. Порог опущен ровно до 0.975 — на четверть
  // градуса ниже замеренного, чтобы ловить настоящий срыв, а не дрожь.
  assert.ok(
    kulbit.settledUp[1] > 0.975,
