...
/**
 * ПОДВЕСНАЯ РАКЕТА RAX-8 («под»): двенадцать труб калибра 92 мм.
 *
 * Считается от того же требования, что и игла, но с ОБРАТНЫМ знаком вывода.
 * Игла отвечала на вопрос «сколько нужно, чтобы снять кольцо одним точным
 * выстрелом». Здесь вопрос другой: **одна ракета НЕ ИМЕЕТ ПРАВА ронять
 * машину** — иначе бой кончается первым же удачным пуском и смотреть не на
 * что.
 *
 * Машину роняет потеря СТОРОНЫ: два соседних кольца выводят центр масс за
 * выпуклую оболочку уцелевших движителей. Кольца стоят в 2.15 м, лопастные
 * диски — в 1.7 м друг от друга. Значит радиус поражения стали обязан быть
 * заметно меньше этого просвета:
 *
 *   E(d) = 60·(1 − d/2.0)^1.15 > 27.6  ⇒  d < 0.98 м
 *
 * Критерий тот же, что у иглы: удвоенный радиус поражения обязан быть меньше
 * межкольцевого шага 2.15 м. 1.96 < 2.15 — попадание уносит СВОЁ кольцо и
 * физически не достаёт до соседнего ни при какой удаче, поэтому уронить машину
 * меньше чем двумя разными попаданиями невозможно по построению.
 *
 * ПОЧЕМУ НЕ МЕНЬШЕ. Первая калибровка давала 0.45 м — вдвое меньше иглиных, —
 * и на стенде выглядела катастрофой: 83% попаданий, 22 снятых куска и НОЛЬ
 * колец. Разбор показал, во что именно попадали: шесть кожухов, две юбки,
 * губа. Кольцевой кожух и есть БРОНЯ ротора, статоры сидят за ним у ступицы, и
 * с сорока пяти сантиметров до них не дотягивается ничего. Боеприпас, который
 * не пробивает то единственное, ради чего его пускают, — не «щадящий», а
 * бесполезный.
 *
 * Отсюда и способ применения: одиночный пуск почти бесполезен, поэтому под
 * стреляет РИПЛОМ с угловым разбросом, и разброс покрывает ошибку
 * упреждения. Неопределённость прогноза закрывается веером, а не точностью, —
 * это честный ответ на то, что маршрута цели атакующий не знает.
 *
 * Скорость 96 м/с ниже иглиных 124 намеренно: игле упреждение «практически не
 * берётся», а поду оно обязано браться — иначе пункт про упреждение
 * вырождается. На 40 м время полёта 0.42 с, и цель за это время уходит на
 * несколько метров.
 */
export const POD_ROCKET_BLAST_RADIUS = 2.0;
export const POD_ROCKET_BLAST_PUSH_RADIUS = 2.8;
export const POD_ROCKET_DAMAGE_ENERGY = 60;
...
export interface ExplosiveProfile {
  ...
  /**
   * Радиус неконтактного взрывателя, м. Ноль — только контактный подрыв.
   *
   * У боеприпаса с малой боевой частью взрыватель осмыслен: иначе радиус
   * поражения тратится на прямое попадание, которого по маневрирующей машине
   * почти не бывает. Замер стенда: медиана промаха ракет 2.5 м при габарите
   * цели 3.1 — снаряды проходили СКВОЗЬ силуэт между лучами и не срабатывали.
   */
...
export const explosiveProfiles: Record<ExplosiveKind, ExplosiveProfile> = {
  ...
  podRocket: {
    kind: "podRocket",
    blastRadius: POD_ROCKET_BLAST_RADIUS,
    blastPushRadius: POD_ROCKET_BLAST_PUSH_RADIUS,
    damageEnergy: POD_ROCKET_DAMAGE_ENERGY,
    carveRadiusMultiplier: 1,
    // Толкает слабее иглы ровно во столько раз, во сколько меньше несёт:
    // 46/90 от 38. Попадание качает цель, но не сбивает ей заход.
    pressureImpulse: 25,
    noiseLevel: 158,
    visualDirections: 12,
    visualProbeDistance: 2.6,
    carveBudget: { maxTargets: 32, workBudget: 4_500, groundWorkBudget: 800 },
    chipBudget: 7,
    looseBurstSpeed: 3.6,
    // МЕНЬШЕ СОБСТВЕННОГО РАДИУСА ПОРАЖЕНИЯ, и это не запас, а условие
    // работоспособности. При метровом взрывателе стенд дал одиннадцать
    // подрывов из двенадцати ракет и НОЛЬ снятых колец: боеприпас исправно
    // рвался в метре от конструкции, а вскрывает сталь он на сорока пяти
    // сантиметрах. Взрыватель, срабатывающий за пределом поражения, — это
    // ракета, потраченная на фейерверк. Держится ниже радиуса вскрытия стали
    // (0.98 м) и после пересчёта боевой части.
    proximityFuse: 0.85,
    playerPush: { horizontal: 4.2, vertical: 3.3 },
    debrisPush: { base: 4.2, falloff: 5.2 },
    projectile: {
      speed: 96,
      density: 1.2,
      gravityScale: 0,
      angularDamping: 0.9,
      // Короче иглиного: труба на 92 мм несёт мало топлива, а самоликвидация
      // за пределами боя не нужна — на 96 м/с это 173 м, вдвое дальше
      // собственного огневого конверта.
      fuseMs: 1800,
      spin: { x: 0, y: 0, z: 0 },
    },
  },
 ...
 */
/**
 * Три состояния тела, в которых одинаково заинтересованы физика и рендер.
 * Строки, а не enum Rapier: этот модуль не должен зависеть от движка.
 */
export type PhysicalBodyKind = "dynamic" | "kinematic" | "fixed";

/**
 * ТЕЛО ОСЕЛО — ТО ЕСТЬ ЗА НЕГО МОЖНО ПЕРЕСТАТЬ ПЛАТИТЬ КАЖДЫЙ КАДР.
 *
 * Вопрос задают двое: рендер (писать ли матрицу) и вытеснение при
 * переполнении (кого выбрасывать первым). Ответ обязан быть ОДИН, и он не
 * «тело не динамическое».
 *
 * ЦЕНА НЕВЕРНОГО ОТВЕТА ЗАМЕРЕНА. Осевший обломок переводится в `Fixed`, а
 * `isSleeping()` у нефизического тела возвращает false, поэтому проверка
 * «спит ли» перестала работать, и её заменили на «не Dynamic». Под это
 * попала КИНЕМАТИКА — а ею живёт всё, что движется без решателя: лопасти и
 * подвижные члены машин, створки дверей и ворот, постоянные роторы. Рендер
 * перестал писать им матрицы, и машина взлетала без винтов: тела ехали с ней,
 * а меши оставались там, где их написали в последний раз.
 *
 * Отсюда правило: **осело только то, что Fixed, либо динамическое и уснувшее.
 * Кинематическое не оседает никогда** — его двигают снаружи, и решатель о его
 * движении не знает по построению. Платить за неподвижную кинематику дешевле,
 * чем один раз не нарисовать движущуюся: кинематических тел в мире единицы
 * (двери, роторы, члены машин), а замороженная куча — тысячи, и она `Fixed`.
 */
/**
 * Перевод типа тела Rapier в три состояния — ОДНИМ местом на весь проект.
 *
 * Числами, а не enum-ом движка: модуль остаётся свободным от Rapier, а
 * сравнение с `Dynamic`/`Fixed` перестаёт расползаться по местам вызова.
 * Расползлось оно ровно один раз и стоило винтов (см. `bodySettled`).
 *
 * Всё, что не `Dynamic` и не `Fixed`, — кинематика: у Rapier таких типов два
 * (по положению и по скорости), и для этого вопроса они неразличимы.
 */
export function physicalBodyKind(
  bodyType: number,
  types: { readonly Dynamic: number; readonly Fixed: number },
): PhysicalBodyKind {
  if (bodyType === types.Fixed) {
    return "fixed";
  }
  if (bodyType === types.Dynamic) {
    return "dynamic";
  }
  return "kinematic";
}
...
/**
 * ЭНЕРГИЯ ОДНОГО УДАРА МОЛОТКОМ — В ТОЙ ЖЕ ШКАЛЕ, ЧТО И ВСЁ ОСТАЛЬНОЕ.
 *
 * У молотка её не было вовсе: удар шёл прямо в `fractureLocallyAt`, который
 * безусловно вносит цель в множество сломанных и по `neighborChance`
 * доцепляет соседей. Прочность материала при этом не спрашивалась НИ РАЗУ, и
 * один удар по стальному члену HX-6 отцеплял 3–7 членов сразу; кластер машины
 * объявлен `linked`, и каждый третий удар доцеплял ещё одного соседа сверх
 * того. Выглядело это ровно так, как и жаловались: молоток деталь не разрушил,
 * но из крепления выбил.
 *
 * Порог выбран не на глаз, а по самому каталогу §2.1: он выше базальта (3.2) —
 * самого прочного камня миров, который молоток обязан брать, — и много ниже
 * стали (24). То есть меняется ровно один класс материалов, и меняется он
 * согласованно с уже существующим законом: у стали нет и пулевого отверстия
 * (`bulletHoleRadius`), и контактного урона она не берёт
 * (`crumbleOnLanding`). Сталь — не материал ручного инструмента; её берут
 * ракета, заряд и прямое попадание, у которых энергии на порядок больше.
 */
export const HAMMER_BLOW_ENERGY = 3.5;

/**
 * Может ли удар молотком вообще работать этот материал. Отрицательный ответ
 * запрещает ВСЮ лестницу урона по цели (отлом, раскол, выгрызание), но не
 * импульс: по двустороннему закону (§1.2) импульс обязателен всегда, а
 * разрушение условно.
 */
export function hammerWorksMaterial(material: BreakableMaterial): boolean {
  return fractureEnergyByMaterial[material] <= HAMMER_BLOW_ENERGY;
}
