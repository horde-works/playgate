 ...
 *
 * ВАЖНАЯ ОГОВОРКА, ДОБАВЛЕННАЯ ЗАМЕРОМ (10.08.2026). Этот кандидат покупает
 * ВСТРЕЧУ, а не выстрел, и путать одно с другим оказалось дорого.
 *
 * Развёртка конвертов (`air-combat-envelope`) показала обратную сторону: ствол
 * жёстко связан с корпусом, вертикальное наведение делается ВЫСОТОЙ, и
 * нерастраченное превышение надо сперва потратить — секундами, в которые
 * машина не стреляет. В лоб с одной высоты выстрел приходит на 0.5 с; с
 * восемнадцати метров превышения — на 5.4; с тридцати восьми не приходит вовсе
 * за девять секунд, и машина проскакивает цель по вертикали, выходя ниже
 * станции.
 *
 * Отсюда правило, которое здесь и надо помнить: пикирование годится ТОЛЬКО
 * против того, кого иначе не встретить. Как ударная связка оно отрицательно.
 *
