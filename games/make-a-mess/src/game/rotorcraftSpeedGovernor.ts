...
/**
 * Политика машины с ВЕКТОРИРУЕМОЙ тягой (подъём ротором: тянет наклоном
 * диска в любую сторону, корпус симметричен). На маршруте нос ей не орган
 * движения, а предпочтение — проектный инвариант «голономный полёт на
 * маршруте не судят по курсу и заносу». Устойчивый вираж, где v/r больше
 * располагаемого темпа рыскания, для неё штатен: нос отстаёт и догоняет
 * после манёвра, управление при этом не теряется. Поэтому маршрутный допуск
 * почти снят (162°) — канал ловит только настоящую потерю управления, — а
 * створ остаётся строгим: прийти надо В ПОЛОЖЕНИИ.
 */
export const VECTORED_SLIP_POLICY: RotorcraftSlipPolicy = {
  enRoute: 0.9 * Math.PI,
