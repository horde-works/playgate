...
export interface RotorcraftTurnCapability {
  ...
  /**
   * РАССЕИВАНИЕ В ВОЗДУХЕ, м/с² — чем машина способна снимать ЭНЕРГИЮ, не
   * меняя траектории: сопротивление на прибранном газу, без колёс и реверса.
   *
   * Поле объявляет только крылатая машина, и объявление включает в общем
   * губернаторе энергетический конверт (см. `pathSpeedCeiling`). Дирижаблю и
   * коптеру он не нужен: их рассеивание — «остановись и виси», мгновенное и
   * бесплатное. У крыла запас — кинетическая ПЛЮС потенциальная энергия, а
   * снимается он только сопротивлением: нельзя одновременно снижаться и
   * гаситься быстрее, чем позволяет drag. Без конверта машина разгонялась на
   * участке, с которого уже не успевала прийти на скорость захода.
   */
...
export interface RotorcraftPathSample {
  ...
  /**
   * НА СКОЛЬКО ТРАССА ВПЕРЕДИ НИЖЕ ТЕКУЩЕЙ ТОЧКИ, м (плюс — снижение).
   * Нужен только машинам с объявленным рассеиванием: у них скорость и высота
   * делят один запас энергии, и снижение впереди — такое же требование к
   * ТЕКУЩЕЙ скорости, как вираж.
   */
...
export function pathSpeedCeiling(
  ...
  for (const sample of samples) {
    ...
    // ── ЭНЕРГЕТИЧЕСКИЙ КОНВЕРТ: ТОЛЬКО ДЛЯ МАШИН С ОБЪЯВЛЕННЫМ РАССЕИВАНИЕМ
    //
    // Кинетическая парабола выше отвечает на вопрос «успею ли замедлиться»,
    // но у крыла это полвопроса: снижение впереди ДОБАВЛЯЕТ энергию, которую
    // тоже придётся снять, и снимается она тем же сопротивлением. Полный
    // закон — обратное распространение энергии от точки впереди:
    //
    //     v² ≤ v_цели² − 2·g·drop + 2·dissipation·d.
    //
    // Ровно ваш случай «разогнался на наборе — не успел погаситься к
    // посадочной»: точка створа со своей скоростью и высотой сама зажимает
    // всё, что до неё, на дистанцию рассеивания. Конверт может только
    // УЖЕСТОЧИТЬ кинетический ответ, никогда не ослабить.
    if (
      capability.dissipation !== undefined &&
      capability.dissipation > 0 &&
      sample.drop !== undefined
    ) {
      const energy =
        target * target -
        2 * 9.81 * Math.max(0, sample.drop) +
        2 * capability.dissipation * distance;
      const energyAllowed = Math.max(target, Math.sqrt(Math.max(0, energy)));
      if (energyAllowed < ceiling) {
        ceiling = energyAllowed;
      }
    }
  }
  return ceiling;
