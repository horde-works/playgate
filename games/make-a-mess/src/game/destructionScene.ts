...
function createEdgewood(): BreakableClusterDefinition[] {
  ...
  // Просвет тропы держали две большие сосны; под причал неба площадку
  // расчистили — обе стояли в габарите мачты и гондолы (см.
  // skyMooringDocument). Внешний ряд севернее и южнее остался, поэтому
  // обрыв из-за стволов по-прежнему не просматривается.
  ...
  const floraPieces: BreakablePieceDefinition[] = [];
  // Два куста из этого рассева стояли ровно под причальной мачтой и её
  // настилом (см. skyMooringDocument): площадку под неё расчистили, а
  // мачту в кусты не ставят.
...
export const openHouseSceneOptions = {
  ...
  // строго на восток — вдаль дороги, в город. Первый кадр: улица уходит
  // к панелькам, причал остаётся за спиной и открывается, если обернуться.
  playerSpawn: [-14.4, 1.3, -12],
