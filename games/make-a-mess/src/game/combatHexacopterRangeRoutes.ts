...
export const combatHexacopterRangeCircuit = createMotionRoute({
  ...
  requirements: {
    ...
    // Скорость на участке выведена из его радиуса: v = sqrt(a·r) при
    // поперечных 6.6 м/с². Прямой разгонный луч радиуса не имеет, поэтому там
    // разрешено всё, что машина берёт синфазной тягой; в виражах — ровно
    // столько, сколько держит наклон.
    speedLimit: ({ progress }) => {
      if (progress < 0.06) return 5;
      if (progress < 0.28) return 24;
      if (progress < 0.46) return 14;
      if (progress < 0.60) return 12;
      if (progress < 0.90) return 11;
      if (progress < 0.96) return 7;
      return 4.5;
    },
  },
  markers: {
