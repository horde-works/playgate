...
/**
 * Состояние захвата указателя глазами кадра.
 *
 * `gestureGiven` — единственное, что отличает просьбу от отчёта. Захват
 * браузер выдаёт только по жесту, и пока жест был (нажали «войти», кликнули по
 * миру), просить нечего: ответ придёт сам — захватом или отказом, а отказ
 * честно уходит в протяжку.
 */
export interface PointerGestureState {
  readonly pointerLockHeld: boolean;
  readonly gestureGiven: boolean;
  readonly fallbackLook: boolean;
}

/**
 * «Кликните, чтобы взять управление» — просьба к игроку, а не отчёт о
 * состоянии браузера.
 *
 * Раньше строка выводилась из одного `!pointerLockHeld`, и это давало два
 * ложных срабатывания. Первое: обычный спавн — игрок только что нажал кнопку,
 * захват ещё летит, а кадр уже просит сделать то, что сделано. Второе, и
 * злее: кадр узнавал о захвате ТОЛЬКО из события `pointerlockchange`, поэтому
 * пересозданный кадр (горячая перезагрузка) считал, что указателя нет, хотя
 * канва держала его — события-то не было. Просьба висела вечно, а клики по
 * миру уходили в удар молотом: менять было нечего, и кадр молчал.
 *
 * Просить нужно ровно там, где игрока поставили в мир без его жеста: прилёт,
 * возврат из пролёта, отпущенный во время рейса указатель.
 */
export function asksForPointerGesture(
