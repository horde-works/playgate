 ...
 *
 * Маршрут авторится в осях машины: `a` — вперёд по носу (на запад вдоль
 * открытого проезда двора), `b` — на левый борт. Он задаёт только линию,
 ...
 * 26 м пути набирает 24 м высоты, то есть под 43°, — потому что двор шириной
 * 11 м зажат между двумя пятиэтажками, и другого коридора у неё нет. Тем же
 * створом она и возвращается: с востока, вдоль проезда, снижаясь на последних
 * 34 метрах.
 *
...
/**
 * Поперечная ось маршрута. Знак важнее, чем кажется: с перевёрнутым базисом
 * круг облёта строится ЗЕРКАЛЬНО — центр острова оказывается не в (30, −15),
 * а в (30, −45), машина всю дорогу тянет к трассе, которой нет, и держит
 * полсотни метров сноса. Формула та же, что у дирижабля города.
 */
