  ...
  if (!holdsTarget) {
    ...
    switch (mode) {
      ...
      case "attack":
        ...
        break;
      case "break":
        // РАЗОРВАЛ — И СРАЗУ ОБРАТНО, если разрыв уже набран.
        //
        // Вердикт Igor (11.08.2026): «он ленивая тварь, долго примеряется,
        // какую-то там позицию себе выстраивает. Где азарт машины, упакованной
        // силами движков по самое небалуйся?»
        //
        // Так и было: разрыв уходил в перепозиционирование ВСЕГДА, и оно
        // держало машину ещё до трёх секунд, даже когда дистанция нового
        // захода уже набрана и заходить можно немедленно. За круг это съедало
        // больше времени, чем сама атака.
        //
        // Перепозиционирование остаётся ровно для своего случая — когда разрыв
        // НЕ набран и надо открыться. Набран — идём на цель.
        if (range >= reattackRange || modeSeconds > 2.2) {
          changeTo("reposition");
        }
        break;
      case "reposition":
        // Полторы секунды вместо трёх: это открывание дистанции, а не пауза
        // на подумать. Не открылся за полторы — значит цель висит рядом, и
        // заходить надо всё равно.
