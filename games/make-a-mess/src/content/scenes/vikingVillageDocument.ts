...
function createSkyLongship(): void {
  ...
  // === Дифферентовочные салазки под сердцем, в тени баллона: дубовый брус
  // вдоль киля и второй поперёк, по ним ходят каменные чушки в кованых
  // обоймах. Это единственный орган, дающий кораблю момент по крену и
  // тангажу: он не тянет, а перевозит настоящий вес, и живой центр масс
  // едет вместе с ним. Обе чушки стоят над измеренным центром масс целой
  // машины, поэтому сами по себе развесовку не портят.
  for (const [axis, y, travel, mass, along] of [
    ["pitch", 6.0, 4.0, 2.0, true],
    ["roll", 6.4, 1.25, 4.7, false],
  ] as const) {
    primitive(ship, `trim:${axis}:rail`, "wood", "cylinder",
      P(0.05, -0.05, y), [0.12, travel * 2 + 0.5, 0.12], "#6b5334", {
        rotation: along ? alongAxis : acrossAxis,
        contactBoxes: [{ position: [0, 0, 0], size: [0.18, travel * 2 + 0.5, 0.18] }],
        // Брус и есть обязательное ядро: перебит — чушка больше не ездит,
        // даже если сама цела.
        actuator: {
          id: `sky-longship:trim:${axis}`,
    ...
    // Чушка висит под брусом в кованой обойме, а не сидит в нём.
    primitive(ship, `trim:${axis}:car`, "stone", "brick",
      P(0.05, -0.05, y - 0.26), along ? [0.5, 0.3, 0.42] : [0.42, 0.3, 0.58], "#6f6a63", {
        rotation: alongAxis,
        // Камень в обойме: объём задан отдельно, иначе чушка такого размера
        // весила бы как пустой ящик.
        volume: mass / 2.55,
        contactBoxes: [{ position: [0, 0, 0], size: [0.56, 0.36, 0.64] }],
        actuator: {
          id: `sky-longship:trim:${axis}`,
          commandChannel: `trim:${axis}`,
          required: true,
