...
export const combatHexacopterRangeDocument: AuthoredSceneDocument = {
  ...
  world: {
    ...
    // НЕБО РАСТЯГИВАЕТСЯ ПОД ТО, ЧТО ДАЛЬШЕ ВСЕГО ЛЕТИТ, — А ЭТО НЕ МАШИНА.
    //
    // Купол рисуется конечным радиусом, и всё, что вышло за него, оказывается
    // нарисовано на пустоте. Пока в мире был один облёт, предел задавала
    // трасса, и ста пятидесяти хватало с запасом.
    //
    // Бой считает иначе, и считает ПРОМАХ. Ракета, не нашедшая цели, летит,
    // пока не кончится взрыватель: 96 м/с × 1800 мс = 173 м ОТ ТОЧКИ ПУСКА, а
    // пуск может случиться на любом выносе. Замер дуэли RAX-8 против VX-8
    // (стенд `air-combat-rig`, 150 с): машины уходили на 144 м от центра,
    // ракеты — на 197. То есть уже сейчас промах покидал небо на полсотни
    // метров, и увидеть это можно было только глазами.
    //
    // Отсюда 330: наибольший вынос машины (144) плюс полная дальность ракеты
    // (173), округлённое вверх. Земля и кромка игрока не тронуты — пешком
    // туда по-прежнему не уйти, выросли только атмосфера и дальность камеры.
    cameraFar: 620,
    center: [0, 0],
    halfExtents: [53, 53],
