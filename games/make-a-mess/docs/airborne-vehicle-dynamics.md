...
**Одно измерение — два вердикта.** Удар порождает ровно одно число, и оно
спрашивается дважды:
...
J       = m_эфф · v_n · (1 + e)                 импульс, обеим сторонам
E       = ½ · m_эфф · v_n² · (1 − e²)           поглощено смятием
```
...
откалиброван в реальных м/с. Энергия решает только МАСШТАБ повреждения и
делится между сторонами по их жёсткости. Подставлять `½mv²` в шкалу урона
запрещено: масса раздута, и раздутие пройдёт насквозь.
...
Гадать «ближайший к центру» нельзя: у широкой машины первым встречает вынос, а
не центр.

**Что происходит с куском машины.** Сталь не крошится — отказывает КРЕПЛЕНИЕ.
Поэтому исход удара для машины это выход куска из compound body по §5.2 со
всеми последствиями: масса, коллайдер и actuator contribution исчезают, кусок
улетает обломком, унося скорость точки отделения. Отдельной «прочности
...
эксплуатационной скорости снижения машины. Порог отказа крепления доказывается
именно этой парой: посадка не ломает, удар ломает.
