...
| Конверт наведения | Порогами коридора, входа в удержание и слияния, выведенными из failure envelope и паспорта машины | Заводить собственные числа отклонения рядом с надзорными |
...
## 6.4. Дифферентовка подвижным грузом
...
Отклонение описывается ОДНОЙ моделью. Источник истины — `VehicleFailureEnvelope`
машины: он говорит, насколько машина вообще имеет право ошибаться. Конверт
наведения выводится из него долями и обязан закрываться раньше:

```text
failure envelope (надзор: «рейс потерян»)
        ↓ доли < 1
guidance envelope (наведение: «исправляю сам»)
        ├── cruise corridor      — невозмущённый полёт
        ├── disturbed corridor   — после измеренного внешнего импульса
        ├── stabilization gate   — вход в удержание и выход из него
        └── merge gate           — доказательство возврата на линию
```

Инварианты:

- каждый предел коридора строго меньше одноимённого предела failure envelope;
  порядок эскалации «коррекция → отказ» является свойством построения, а не
  совпадением двух рукописных таблиц;
- известный импульс может только сузить коридор, никогда не расширить;
- удержание входит раньше, чем теряется коридор, и выходит с гистерезисом;
- merge gate — это ужатый approach gate ТОЙ ЖЕ машины, а не общая константа;
- вертикальный порог удержания берётся из располагаемой дифферентовки
  (`g × liftTrimRange`), поэтому мелкая гондола и длинный состав получают
  разные числа без авторской правки.

Машина может отклониться от выведенного конверта только тремя физическими
множителями: масштабом коридора, масштабом merge gate и фактической
вертикальной скоростью, которую гасит её трим. Отдельных таблиц допусков в
общем корректоре быть не должно.

## 10.7. Бюджет попыток

Попытка коррекции без критерия исчерпания — не попытка. Поэтому:

- каждая коррекция считается: и на круге, и на глиссаде;
...
- на время коррекции маршрутные и позиционные таймеры надзора приостановлены —
  но не дольше суммарного `correctionGraceSeconds` за рейс. Исчерпав бюджет,
  watchdog возобновляет счёт, даже если корректор ещё работает;
