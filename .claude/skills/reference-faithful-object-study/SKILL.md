...
description: >-
  Research, engineer, implement, test, and render reference-faithful 3D objects
  as canonical code-authored geometry before scene integration — both
  real-world types and imagined machines. Use for Object Lab / object-study
  work on vehicles, cars, aircraft, rotorcraft, drones, hexacopters, boats,
  ships, windmills, machines and articulated mechanisms (rotors, wheels,
  canopies, doors, gates, sails, turrets, kinematic groups, swept envelopes),
  and equally for buildings, architectural props, yard or street objects,
  sheds, pumps, furniture, equipment, openings, supports and joints. Also use
  for ImageGen-driven concept objects where a generated multi-view sheet
  supplies visual character, for blueprint/measured-drawing work, multi-angle
  PNG studies and geometry audits — whenever an image must correspond exactly
  to the object that will later exist in the world rather than serve as a
  concept. Triggers: ObjectLabModel, objectModel.ts, object lab, evidence
  card, discrepancy log, canonical object, motionConstraints, rotor pivot,
  swept envelope, cutaway, silhouette control, model hash, capture manifest.
---
...
4. Read [articulation.md](references/articulation.md) completely when the object has any moving part, any historically moving part (even if motion is frozen this milestone), or any kinematic group to bound: rotors, wheels, canopies, leaves, gates, sails, caps, turrets.
5. Read [imagination-pipeline.md](references/imagination-pipeline.md) completely when the object is imagined/fictional or when ImageGen output participates anywhere in the pipeline.
6. If the repository uses `ObjectLabPart`, TypeScript object studies, fixed capture scripts, group filtering, or this Playgate pipeline, also read [object-lab-typescript.md](references/object-lab-typescript.md) completely.
7. Read [casebook.md](references/casebook.md) when the object shares a form or risk with a recorded case. Use its failures as regression tests, not as dimensions for a new object.
8. Use [templates.md](references/templates.md) for the evidence card, discrepancy log, README index and handoff.
...
### 4. Decompose the object into constructible parts
...
### 5. Implement canonical geometry
...
### 6. Write independent tests before trusting the render
...
### 7. Capture the canonical object
...
### 8. Run the autonomous discrepancy loop
...
### 8.1 Verify authored lighting as construction
...
### 9. Verify and request visual acceptance
...
4. check the material adapter by semantic ids; list all transparent prefab pieces and reject any that is not real glass, membrane or another physically transparent material;
5. check intact and dynamic renderers;
6. prove the support graph and destruction;
7. only then place the prefab in the world and check environment scale;
8. capture a world view set without cancelling the lab set.
